...
class LocalAlign(QWidget):
    ...
    def start_alignment(self, input_seq1, input_seq2, label_1, label_2):
        """Implements the methods on class SmithWatermanAlgorithm"""
        try:
            self.localign = SmithWatermanAlgorithm(
                scoring_sys, seqA=input_seq1, seqB=input_seq2
            )
            if self.localalign.is_valid:
                result = self.localalign.result_alignment()
            else:
                raise ValueError()
        except ValueError as e:
            self.pop_warning(f"Local Alignment Error: {e}")

    def result_alignment(sample_A, sample_B):
